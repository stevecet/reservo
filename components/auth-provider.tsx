...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const token = document.cookie.includes("admin-token=valid") || document.cookie.includes("user-token=valid")
    ...
    if (token && role) {
      ...
      setUser({
        ...
        email: role === "admin" ? "admin@bookinghub.com" : "user@example.com",
        role,
        verified: true,
      })
  ...
  const login = async (email: string, password: string, role: UserRole): Promise<boolean> => {
    ...
    if (role === "admin" && email === "admin@bookinghub.com" && password === "admin123") {
